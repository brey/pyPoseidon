...
def out2verif(t1,t2,name,iloc):

 obsname='/mnt/pandora/Users_Critech/Brey/{}.mat'.format(name)
...
# #### read cleaned obs
 ...
 obs=pd.DataFrame({'time':mat['times'].flatten(),'s':mat['ss_{}'.format(name)].flatten(), 'offset':mat['offset'].flatten()})
...
# #### output
 ...
 with open(SAVEPATH+'/{}.csv'.format(name), 'w') as f:
    f.write('# variable: s\n')
    f.write('# units: $m$\n')
    f.write('date offset obs fcst\n')

 with open(SAVEPATH+'{}.csv'.format(name), 'a') as f:
